...
When a user performs any action — clicking a button, submitting a form, or searching — a chain of events is triggered across multiple systems. This coordinated system is called **full stack architecture**.

### 🍽️ Real-World Analogy — A Restaurant
...
User Types URL / Clicks Button
   ...
   Frontend (UI)            ← Builds and sends the API request
          |
          v
     API Layer              ← Routes request to the right backend handler
          |
          v
   Backend (Logic)          ← Validates, processes, applies rules
          |
          v
...
Database → Backend → API → Frontend → Browser → User sees result
```

> **Key Insight:** Every single thing you do on a website triggers this full cycle — even just loading a page.

---

## 2. Full Stack Architecture Overview

A full stack application is built in **distinct layers**, each with a well-defined responsibility. No layer does the job of another.
...
The browser renders the visual interface and executes JavaScript.
...
| **Execute JavaScript**        | Runs client-side logic                           |
| **Send HTTP requests**        | Forwards login credentials, form data, etc.      |
...
The frontend is everything **visible to the user** — text, images, buttons, forms, animations.

**Core responsibilities:**
- Render content received from the backend
- Handle user input events (clicks, keypresses, form submissions)
- Send API requests when user takes an action
...
The **API** (Application Programming Interface) is the **contract** between the frontend and backend. It defines what requests can be made and what responses to expect.
...
| Task                       | Description                                          |
|----------------------------|------------------------------------------------------|
| **Parse & validate requests** | Ensure incoming data meets required format and rules |
| **Authenticate users**     | Verify identity using sessions, tokens, or cookies   |
| **Authorize actions**      | Check whether the user has permission               |
...
Django                -->  Python framework; built-in admin, auth, ORM
...
The database is the **permanent storage layer**. All data that must survive beyond a single session is stored here.
...
This section traces the complete journey of a **login request** from click to access granted.
...
| 3    | **Backend**   | Validates format and presence of the fields               |
| 4    | **Backend+DB**| Queries the database to verify credentials                |
| 5    | **Backend**   | If valid, creates a session and stores user data          |
...
| 9    | **Backend**   | Reads the cookie, looks up the session, grants access     |

---

## 5. What is a Server?

A **server** is a computer (physical or virtual) that **continuously listens for incoming requests** from clients (browsers, mobile apps) and sends back appropriate responses.
...
|------------------------------|--------------------------------------------------|
| **Listen for requests**      | Waits for any client to send `POST /login`       |
| **Run backend code**         | Executes Node.js or Python logic                 |
| **Communicate with DB**      | Runs a SQL query or MongoDB find operation       |
| **Send structured response** | Returns `{ success: true, user: { ... } }`       |

> A server is not a single machine — it can be a **cluster**, a **cloud function**, or a **containerized process**.

---

## 6. Rate Limiting

**Rate limiting** restricts the number of requests a client (user, IP, or API key) can make within a defined time window.

### Why It Matters

Without rate limiting, a bot could send **thousands of requests per second** to crash or abuse a server.
...
User --> Server --> Database --> Server --> User
         (Query executed EVERY single time — slow!)

With Cache:
User --> Server --> Cache (data found) --> User            [Fast ⚡]
User --> Server --> Cache (data absent) --> Database --> Cache --> User
                                              (Only first time — cached for next!)
```
...
|-----------------|-----------------------------------------------------------|--------------|
| **Cache Hit** ✅ | Requested data found in cache; returned immediately       | Very fast    |
| **Cache Miss** ⚠️ | Data not in cache; fetched from database, then stored    | Slower once, fast on retry |

### Benefits of Caching

- ⚡ Dramatically reduces average response times
- 🔓 Reduces load on the database
- 👥 Allows serving more users simultaneously
- 😊 Near-instant response for users
...
A **cookie** is a small piece of data (text string) that a server sends to the browser. The browser stores it and **automatically sends it back** with every subsequent request to the same server.
...
**Session-based authentication** tracks and verifies authenticated users across multiple requests using sessions and cookies.
...
Server        = Machine or process that listens for requests and responds
Authentication = Verifying WHO the user is
Authorization  = Verifying WHAT the user is allowed to do
```

### Flash Cards — Key Differences

| Comparison | A | B |
|------------|---|---|
| **Cookie vs Session** | Stored in **browser** | Stored on **server** |
| **Cache Hit vs Miss** | Data **found** in cache (fast) | Data **not found**, hits DB |
| **Authentication vs Authorization** | **Who** are you? | **What** can you do? |
| **GET vs POST** | **Retrieve** data | **Send / Submit** data |
